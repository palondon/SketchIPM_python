...
#------------------------------------------------------------
#import gen_data 
#from parameters import m,n,p,w,gamma,sigma_step,sigma,t_iter,tol_ipm,MAXIT_cg,tol_cg
from parameters import p,gamma,sigma_step,sigma,t_iter,tol_ipm,MAXIT_cg
...
def ipm(m,n,w,A,b,c,tol_cg):
	...
	error_pd = 1.0
	#error_pd_new = 0.0
	while (error_pd > tol_ipm): # & (error_pd_old >= error_pd_new)):   #for k in range(t_iter):
		print '\n------------------------------------------'
		print 'k = ', k
		print '\nw = ', w
		print 'tol_cg = ', tol_cg
		...
		# condition number 
		kap_ADW_vec.append(np.linalg.cond(lhs))
...
def ipm_standard(m,n,A,b,c,tol_cg):
	...
	#kap_AD_vec = np.zeros(size_vec)
	k = 0 	#t_iter = 0
	error_pd = 1
	while error_pd > tol_ipm:   
	#for k in range(1):
		print '\nk = ', k
		#-------------------------------------------------
		# (a) calculate residuals wrt the current point (x,y,s)
		...
		kap_AD_vec.append(np.linalg.cond(lhs))
		#kap_AD_vec[k] = np.linalg.cond(lhs)
		...
		AA = sp.sparse.linalg.LinearOperator((m,m), matvec=mv, rmatvec =rmv)
		...
		t_cg_2 = time.time()
		print 'iter_cg_stan= ', iter_in_cg_k
